import React, { useState } from 'react';
// import { logger } from '@/lib/logger';
// const log = logger.child({ module: "rss-button", isClient: true });

const RssFeedCopyButton = ({ rssUrl,size = 24, color = "#FFA500"  }: { rssUrl: string, size?: number , color?: string }) => {
  const [copySuccess, setCopySuccess] = useState(false);

  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(rssUrl);
      setCopySuccess(true);
    } catch (err) {
      console.error(err);
      setCopySuccess(false);
    }

    // Reset the copy success message after a few seconds
    setTimeout(() => setCopySuccess(false), 3000);
  };

  return (
    <div className="flex items-center">
      <button
        onClick={copyToClipboard}
        className={`flex items-center p-2 rounded-md border border-gray-300 hover:bg-gray-200 transition duration-200 ${
          copySuccess ? 'bg-green-200' : 'bg-white'
        }`}
      >
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 448 512"
      width={size}
      height={size}
    >
      <path
        fill={color}
        d="M64 32C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64H384c35.3 0 64-28.7 64-64V96c0-35.3-28.7-64-64-64H64zM96 136c0-13.3 10.7-24 24-24c137 0 248 111 248 248c0 13.3-10.7 24-24 24s-24-10.7-24-24c0-110.5-89.5-200-200-200c-13.3 0-24-10.7-24-24zm0 96c0-13.3 10.7-24 24-24c83.9 0 152 68.1 152 152c0 13.3-10.7 24-24 24s-24-10.7-24-24c0-57.4-46.6-104-104-104c-13.3 0-24-10.7-24-24zm64 120c0 17.7-14.3 32-32 32s-32-14.3-32-32s14.3-32 32-32s32 14.3 32 32z"
      />
    </svg>
       Copy RSS URL
      </button>
      {copySuccess && (
        <p className="ml-3 text-green-600 transition-opacity duration-300">
          Copied!
        </p>
      )}
    </div>
  );
};

export default RssFeedCopyButton;
